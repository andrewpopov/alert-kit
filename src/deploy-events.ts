...
const DEPLOY_MONITOR_STATUSES: readonly DeployMonitorStatus[] = ['ok', 'warn', 'crit'];
const DEPLOY_MONITOR_KINDS: readonly DeployMonitorKind[] = ['alert', 'escalation', 'reminder', 'recovery'];
 ...
 */
export function severityFromDeployStatus(status: DeployMonitorStatus, kind: DeployMonitorKind): Severity {
...
function titleFor(alert: DeployMonitorAlert, host: string): string {
 ...
 */
function alertFromDeployAlert(event: DeployMonitorEvent, alert: DeployMonitorAlert): Alert {
...
function validateAlert(value: unknown, index: number): string | null {
  if (!isPlainObject(value)) return `alerts[${index}] is not an object`;
  if (typeof value.id !== 'string') return `alerts[${index}].id must be a string`;
  if (typeof value.kind !== 'string' || !DEPLOY_MONITOR_KINDS.includes(value.kind as DeployMonitorKind)) {
    return `alerts[${index}].kind must be one of ${DEPLOY_MONITOR_KINDS.join(', ')}`;
  }
  if (typeof value.status !== 'string') return `alerts[${index}].status must be a string`;
  if (typeof value.message !== 'string') return `alerts[${index}].message must be a string`;
  return null;
}
 ...
 */
export function parseDeployMonitorEvent(json: string): ParseDeployMonitorEventResult {
  let raw: unknown;
  try {
    raw = JSON.parse(json);
  } catch (err) {
  ...
  if (typeof raw.eventId !== 'string') return { ok: false, error: 'eventId must be a string' };
  ...
  if (typeof raw.host !== 'string') return { ok: false, error: 'host must be a string' };
  if (!Array.isArray(raw.alerts)) return { ok: false, error: 'alerts must be an array' };
  ...
  return {
    ...
    event: {
      ...
      host: raw.host,
      alerts: raw.alerts as DeployMonitorAlert[],
    },
  };
}
