 ...
 *
 * alert-kit has ZERO runtime dependencies (global `fetch` only), so the
 * throwaway consumer installs only the tarball.
 *
...
const workDir = mkdtempSync(join(tmpdir(), 'alert-verify-'));
let tarballPath;
...
try {
  ...
  writeFileSync(
    ...
    JSON.stringify({ name: 'alert-kit-consumer', version: '1.0.0', private: true }, null, 2),
  );
